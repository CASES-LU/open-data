...
#
# Install the Open data stats service with its dependencies and enable a new systemd service.
#
...
STATS_PATH="/home/$ME/open-data"
STATS_HOST='0.0.0.0'
STATS_PORT='5010'
STATS_DB_NAME='opendata'
...
git clone https://github.com/CASES-LU/open-data $STATS_PATH
cd $STATS_PATH
npm install
...
sudo bash -c "cat << EOF > /etc/systemd/system/opendata.service
[Unit]
Description=Open data stats service
